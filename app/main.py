...
def create_invoice_api(data: InvoiceCreate, db: Session = Depends(get_db)):
...
def submit_invoice_api(invoice_id: int, db: Session = Depends(get_db)):
...
def withdraw_invoice_api(invoice_id: int, db: Session = Depends(get_db)):
...
def approve_invoice_api(invoice_id: int, db: Session = Depends(get_db)):
    invoice = approve_invoice(db, invoice_id, user_id=2)
    return APIResponse(
        status="success",
...
def send_invoice_api(invoice_id: int, db: Session = Depends(get_db)):
...
def mark_invoice_paid_api(invoice_id: int, db: Session = Depends(get_db)):
